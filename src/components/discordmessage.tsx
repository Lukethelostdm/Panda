        ...
        <div className="w-full flex items-start justify-start">
            ...
            <div className="w-full max-w-xl">
                <div className="flex items-center">
                    ...
                    <span className="ml-2 px-1.5 py-1.5 text-xs font-semibold bg-brand-600 text-white rounded">
                        APP
                    </span>
                    <span className="text-gray-100 ml-1.5 text-xs font-normal">
                        {timeStamp}
                    </span>
                </div>
                ...
                <div className="bg-[#2f3136] text-sm w-full rounded p-3 mb-4 mt-1.5">
                    <div className="flex  flex-row items-center justify-between mb-2">
                        {badgeText ? <span className={cn("inline-flex order-2 items-center rounded-md px-2 text-xs font-medium ring-1 ring-inset", getBadgeStyles (badgeColor) )}>{badgeText}</span> : null}
                        <p className="text-white order-1 text-base/7 font-semibold">{title}</p>
                    </div>

                    {Object.entries(content).map(([key, value]) => (
                        <p key={key} className="text-[#dcddde] text-sm/6">
                            <span className="text-[#b9bbbe] mx-2 items-start">{key}:</span>{value}
                        </p>
                    ))}
